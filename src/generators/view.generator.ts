...
export class ViewGenerator implements INodeGenerator
{
    public name = "ASP MVC View Generator";
    public replaceExisting = true;
    public version = "1.0.0";

    public canHandleType(node: INode): boolean {
        return node.type.id == ViewNode.NodeType.id;
    }
    ...
    public computeFileLocation(node: ViewNode, group: NamespaceNodeGroup, project: AspMvcProject): string {
        return `${project.outputDirectory}/${group.name}/Views/Shared/${node.data.name}.cshtml`;
    }
}